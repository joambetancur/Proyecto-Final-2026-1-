# Virus-Run: Infiltración en el Kernel Sintético

Proyecto académico para la asignatura **Informática II (2026-1)** de la **Universidad de Antioquia**. Este desarrollo consiste en un simulador interactivo programado en C++ que integra modelos físicos y clases para representar una persecución digital en un entorno de software.

## 📌 Propósito del Proyecto
El objetivo es diseñar una experiencia de juego tipo *runner* donde un **Código Malicioso** debe evadir los protocolos de limpieza de un **Antivirus**. 

## 🧩 Ejes Temáticos de Implementación
* **C++:** Uso de punteros inteligentes, estructuras de control de flujo y optimización de memoria.
* **Paradigma de Objetos (POO):** Modelado de entidades mediante clases.
* **Motores Físicos:** Programación de trayectorias parabólicas, oscilaciones y rebotes.
* **Interfaz Gráfica (Qt)** 

> [!TIP]
> Puedes consultar la lógica extendida de los niveles en la carpeta `momento_1/`.

## 💻 Stack Tecnológico
* **Lenguaje base:** C++ (Estándar 11/14/17).
* **Framework:** Qt 5.x / 6.x.
* **IDE Recomendado:** Qt Creator.
